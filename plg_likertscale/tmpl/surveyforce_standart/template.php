    		...
    		//question option rows
    				for ($i = 0; $i < $mcount; $i++) {
    					...
    					for ($j = 0; $j < $scount; $j++) {
    						...
    						$return_str.=
    							'<td class="ls_answer_cell">'
    							.'<input class="ls_radio" type="radio" name="quest_radio_'.SF_LikertscaleTemplate::$question->id.'_'.$mfield[$i]['mfield_id'].'" value="'.$sdata[$j]['sfield_id'].'" '.$checked.'>'
    							.'</td>';
    					}
    					$return_str.='</tr>';
    					$k = 3 - $k;
