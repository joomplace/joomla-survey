...
if ($saveOrder) {
    $saveOrderingUrl = 'index.php?option=com_surveyforce&task=iscales.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_surveyforce&view=iscales'); ?>" method="post" name="adminForm" id="adminForm">
        ...
        <table class="table table-striped" id="testimonialsList">
            ...
            <tbody>
                ...
                foreach ($this->items as $i => $item) :
                    ...
                    $canEdit = $user->authorise('core.edit', $extension . '.iscales.' . $item->id);
                    $canCheckin = $user->authorise('core.admin', 'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
                    $canChange = $user->authorise('core.edit.state', $extension . '.iscales.' . $item->id) && $canCheckin;
                    ?>
                    <tr class="row<?php echo $i % 2; ?>" sortable-group-id="1">
                        ...
                        <td class="nowrap ">
                                ...
                                <?php if ($canEdit) : ?>
                                    <a href="<?php echo JRoute::_('index.php?option=com_surveyforce&task=iscale.edit&id=' . $item->id); ?>"><?php echo $this->escape($item->iscale_name); ?></a>
                                <?php else : ?>
                                    <?php echo $this->escape($item->iscale_name); ?>
                                <?php endif; ?>
