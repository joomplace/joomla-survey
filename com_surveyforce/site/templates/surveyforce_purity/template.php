...
if (!class_exists('surveyforce_template_class')) {
    ...
    class surveyforce_template_class {
        ...
        function SF_MainLayout() {
            global $mosConfig_live_site, $sf_lang;
			$mosConfig_live_site = JUri::root();
            $document = & JFactory::getDocument();
            $document->addScript('http://ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js');
            $document->addScript(JUri::root() . 'components/com_surveyforce/templates/surveyforce_purity/choosen.js');
            $document->addStyleSheet(JUri::root() . 'components/com_surveyforce/templates/surveyforce_purity/choosen.css');
			...
			<link rel="stylesheet" type="text/css" href="{$mosConfig_live_site}components/com_surveyforce/templates/surveyforce_purity/surveyforce.css"/>
			<!--[if lte IE 8]> <link rel="stylesheet" href="{$mosConfig_live_site}components/com_surveyforce/templates/surveyforce_purity/ie.css" type="text/css"/><![endif]-->

			<div id="surveyforce" class="contentpane surveyforce panel panel-primary">
			<div class="panel-heading"><h2 class="panel-title text-center">{SURVEY_NAME}</h2></div>
...
EOF_RES;
        ...
        $str = '<div id="progress" style="display:none">
					<div class="progress progress-striped">
						...
						<div id="progress_bar_txt">' . JText::_('COM_SURVEYFORCE_PROGRESS') . ' 0%</div>
					</div>
				</div>';
...
EOFTMPL;
        ...
        function SF_GetElements($sf_config) {
            ...
            <script language="javascript" type="text/javascript"><!--//--><![CDATA[//><!--
                    preload([
                    '<?php echo JURI::base() ?>components/com_surveyforce/templates/surveyforce_purity/images/check_off.png',
                    '<?php echo JURI::base() ?>components/com_surveyforce/templates/surveyforce_purity/images/check_on.png',
                    '<?php echo JURI::base() ?>components/com_surveyforce/templates/surveyforce_purity/images/custom_select.png',
                    '<?php echo JURI::base() ?>components/com_surveyforce/templates/surveyforce_purity/images/radio_off.png',
                    '<?php echo JURI::base() ?>components/com_surveyforce/templates/surveyforce_purity/images/radio_on.png'
                ]);
                    ...
                    for (i = 0; i < mcount; i++) {
                        ...
                        for (j = 0; j < scount; j++) {
                            ...
                            return_str = return_str +
                                    '<td class="ls_answer_cell" onclick="javascript: sf_getObj(\'quest_radio_' + questions[n].cur_quest_id + '_' + SF_getElement(data, 'mfield_id', i) + '_' + j + '\').checked=\'checked\'; check_answer(' + n + '); ">' +
                                    ...
                                    '<label class="jb_survey_label" for="quest_radio_' + questions[n].cur_quest_id + '_' + SF_getElement(data, 'mfield_id', i) + '_' + j + '">' + SF_getElement(data, 'sfield_text', j) + '</label>' +
                                    '</td>';
                        }
                        return_str = return_str + '</tr>';
                        k = 3 - k;
                    ...
                    if (acount > 0) {
                        ...
                        if (quest_style == 1) {
                            return_str = return_str +
                                    ...
                                    '</select>' +
                                    '<br/>' +
                                    '<?php echo $sf_lang['SF_OTHER_ANSWER']; ?>' +
                                    '<br/>' +
                            ...
                            return_str = return_str + '<tr>' +
                                    ...
                                    '<label style="display:inline;position:relative;left:-18px;" for="quest_radio' + questions[n].cur_quest_id + 'e">' + SF_getElement(data, 'afield_text', 0) + '</label>' +
                                    '<input class="po_other" type="text" id="other_op_' + questions[n].cur_quest_id + '" name="other_op_' + questions[n].cur_quest_id + '" value="' + other_val + '"/>' +
                                    '</td>' +
                                    '</tr>';
                        }
                ...
                function getPickMany(n) {
                    ...
                    if (acount > 0) {
                        ...
                        return_str = return_str + '<tr>' +
                                ...
                                '<label style="display:inline;position:relative;left:-18px;" onclick="javascript: return check_num_opt(' + n + ', this)" for="quest_check' + questions[n].cur_quest_id + 'e">' +
                ...
                function getShortAnswer(n) {
                    ...
                    if (quest_inp_count == 0) {
                        ...
                        for (i = 0; i < quest_inp_count; i++) {
                            ...
                            xn_pos = questions[n].cur_quest_text.indexOf('{xn}');
                            y_pos = questions[n].cur_quest_text.indexOf('{y}');

							if((xn_pos < y_pos || y_pos == -1) && (xn_pos < x_pos || x_pos == -1) && xn_pos != -1){
                                tmp_str = '<input class="sa_input_text" type="number"  onkeypress="return event.charCode >= 48 && event.charCode <= 57" id="short_ans_' + questions[n].cur_quest_id + '_' + i + '" name="short_ans_' + questions[n].cur_quest_id + '_' + i + '" value="' + ans_text + '" />';
                                questions[n].cur_quest_text = questions[n].cur_quest_text.replace(/\{xn\}/, tmp_str);
							}else if ((x_pos < y_pos || y_pos == -1) && x_pos != -1) {
                ...
                function getDropDown(n) {
                    ...
                    var i = 0;
                    for (i = 0; i < mcount; i++) {
                        make_select[i] = '<option class="dd_option" value="0"><?php echo $sf_lang['SURVEY_DROP_DOWN_FIRST_ELEMENT'] ?></option>';
                    }
                    var j = 0;
                    var selected = '';
                    var jj = 0;
                    ...
                    for (i = 0; i < mcount; i++) {
                        return_str = return_str + '<tr>' +
                                ...
                                '<select onchange="javascript: check_answer(' + n + ');" class="chzn-select dd_select" name="quest_select_' + questions[n].cur_quest_id + '_' + SF_getElement(data, 'mfield_id', i) + '">' + make_select[i] +
                                '</select>' +
                                '</td>' +
                                '</tr>';
                    }
                ...
                function getDragDrop(n) {
                    ...
                    for (i = 0; i < mcount; i++) {
                        ...
                        return_str = return_str + '<tr>' +
                                '<td width="2%">' +
                                '<div class="jb_survey_dragdrop_left" id="cdiv' + questions[n].cur_quest_id + '_' + (i + 1) + '" style="position:relative; width:250px; height:30px; margin:10px; background-position:right; background-color: #<?php echo $sf_config->get('color_cont') ?>; border: 1px solid #000; cursor:default; text-align:center; vertical-align:middle;">' +
                                SF_getElement(data, 'mfield_text', i) +
                                '</div>' +
                                '</td>' +
                                '<td width="auto">' +
                                '<div class="jb_survey_dragdrop_right" id="ddiv' + questions[n].cur_quest_id + '_' + (i + 1) + '" onmousedown="startDrag(event, ' + n + ');" onmouseup="stopDrag(event, ' + n + ');" style="position:relative; width:250px; height:30px; margin:10px; background-position:left; background-color: #<?php echo $sf_config->get('color_drag') ?>; border: 1px solid #000; cursor:default; text-align:center; vertical-align:middle;">' +
                ...
                function getRanking(n) {
                    ...
                    var i = 0;
                    for (i = 0; i < mcount; i++) {
                        make_select[i] = '<option class="r_option" value="0"><?php echo $sf_lang["SURVEY_RANK_FIRST_ELEMENT"] ?></option>';
                    }
                    var j = 0;
                    var selected = '';
                    var jj = -1;
                    ...
                    for (i = 0; i < mcount; i++) {
                        ...
                        return_str = return_str + '<tr>' +
                                ...
                                '<select class="chzn-select r_select" onchange="javascript:removeSameRank(this, ' + n + ');" name="quest_select_' + questions[n].cur_quest_id + '_' + SF_getElement(data, 'mfield_id', i) + '" id="quest_select_' + questions[n].cur_quest_id + '_' + SF_getElement(data, 'mfield_id', i) + '">' +
                                make_select[i] +
                                '</select>' +
                                '</td>' +
                                '</tr>';
                    ...
                    if (iscount > 0) {
                        ...
                        for (j = 0; j < iscount; j++) {
                            ...
                            return_str = return_str + '<td class="i_ans_cell" onclick="javascript: sf_getObj(\'iscale_radio' + questions[n].cur_quest_id + '_' + j + '\').checked=\'checked\';">' +
                                    ...
                                    '<label class="jb_survey_label" for="iscale_radio' + questions[n].cur_quest_id + '_' + j + '">' + SF_getElement(data, 'isfield_text', j) + '</label>' +
                                    '</td>';
                        }
                        return_str = return_str + "<\/tr>";
                        return_str = return_str + "<\/table><\/form><\/div>";
                ...
                function getButton(type, label, onclick) {
                     ...
                     */
                    var return_str = '<input type="button" id="sf_' + type + '_button" class="btn btn-primary" value="' + label + '" onclick="' + onclick + '" />';
                    return return_str;
                }
                //--><!]]>
            </script>
        ...
        function SF_GetStartButton() {
            global $sf_lang;
            return '<input type="button" id="sf_start_button"  value="' . JText::_('COM_SURVEYFORCE_START') . '" class="btn btn-primary" onclick="javascript: sf_StartSurveyOn()"/>';
        }
