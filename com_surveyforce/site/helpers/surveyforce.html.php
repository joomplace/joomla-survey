...
class survey_force_html 
{
	function processTemplate($template, $sf_config, $survey, $show_description) {
		...
		if ($survey->sf_image) {
			$template = preg_replace('/\{BACKGROUND_IMAGE\}/', "background: url(images/com_surveyforce/".$survey->sf_image.") no-repeat;", $template, 1);
		} 
		$template = preg_replace('/\{PROGRESS_BAR\}/', survey_force_html::get_progress_bar($sf_config), $template,1);		
		$template = preg_replace('/\{ERROR_MESSAGE_TOP\}/', survey_force_html::get_error_message_top($sf_config), $template,1);
		$template = preg_replace('/\{ERROR_MESSAGE_BOTTOM\}/', survey_force_html::get_error_message_bottom($sf_config), $template,1);
	...
	?>		
	<script language="JavaScript" src="<?php echo $mosConfig_live_site?>/components/com_surveyforce/bits_message.js" type="text/javascript"></script>

    <script language="JavaScript" src="<?php echo $mosConfig_live_site?>/components/com_surveyforce/jquery.js" type="text/javascript"></script>
    <script language="JavaScript" src="<?php echo $mosConfig_live_site?>/components/com_surveyforce/pagination.js" type="text/javascript"></script>
	<script language="JavaScript" type="text/javascript">
	<!--//--><![CDATA[//><!--
		var mes_not_avail = '<?php echo JText::_('SURVEY_NOT_AVAIL'.strtoupper($message))?>';
		function sf_AddEvent(obj, evType, fn){
			if (obj.addEventListener){ obj.addEventListener(evType, fn, true); return true;
			} else if (obj.attachEvent){ var r = obj.attachEvent("on"+evType, fn); return r;
			} else { return false; }
		}
		function do_show_mes() { ShowMessage('error_messagebox', 1, mes_not_avail); }
		function sf_SetTimer() { timerID = setTimeout("do_show_mes()", 300); }
	...
	<tr><td colspan="3" align="center" style="text-align:center "><div style="text-align:center;">Powered by <span title='JoomPlace'><a target='_blank' title='JoomPlace' href='http://www.joomplace.com/e-learning/surveyforce-deluxe.html' rel="nofollow">Joomla component</a></span> SurveyForce Deluxe Software</div></td></tr>
	<?php }?>
	</table>
...
function sf_MakeRequest(url) {
    ...
    if (window.XMLHttpRequest && !http_request) { // Mozilla, Safari,...
...
function sf_CreateQuestions() {
	...
	for (i = 0; i < quest_count; i++){		
		...
		if(questions[i].cur_quest_type == 5 || questions[i].cur_quest_type == 9){
			if(window.getChoosenSelect) getChoosenSelect();
		}
	}
}
			...
			switch (task) {
				...
				case 'finish':
					...
					if(parseInt(redirect_enable)){
						...
						if(fpage_redirect_url) setTimeout("SF_do_redirect('" + fpage_redirect_url + "')", fpage_redirect_delay * 1000);
					}
...
//--><!]]></script>
	<?php
		...
		echo survey_force_html::processTemplate($template, $sf_config, $survey, $survey->sf_enable_descr);
