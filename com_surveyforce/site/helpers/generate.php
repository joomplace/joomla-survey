...
class sf_ImageGenerator
{
    ...
    function getBarChartImage($options_v, $options, $filename)
    {
        ...
        $barchart->setFontProperties(JPATH_SITE . "/components/com_surveyforce/assets/pchart/Fonts/tahoma.ttf", $font_size);
        ...
        // Draw the 0 line
        $barchart->setFontProperties(JPATH_SITE . "/components/com_surveyforce/assets/pchart/Fonts/tahoma.ttf", 6);
	...
	function getImage($survey_id = 0, $quest_id = 0, $start_id = 0)
    {
        ...
        if ($quest_id && $survey_id && $start_id) {
            ...
            $database->SetQuery($query);
            $start_ids_res = $database->loadRowList();
            $start_ids = array();

            foreach ($start_ids_res as $ids)
                array_push($start_ids, $ids[0]);

            switch ($qtype) {
                case '1':

                    $query = "SELECT id, ftext from #__survey_force_fields WHERE quest_id = $quest_id ORDER BY `ordering`";
                    $database->SetQuery($query);
                    ...
                    $tmp = new stdClass();
                    $tmp->id = 0;
                    $tmp->stext = JText::_('COM_SURVEYFORCE_NO_ANSWER');
                    $fields[] = $tmp;
                    ...
                    $database->SetQuery($query);
                    $maxval = count($database->loadRowList());
                    ...
                    foreach ($questions as $question) {
                        ...
                        $database->SetQuery($query);
                        $ans_fields = $database->loadColumn();
                        $j = count($ans_fields);
                ...
                case '2':
                    ...
                    $database->SetQuery($query);
                    $answers_res = $database->loadRowList();
                    ...
                    $tmp = new stdClass();
                    $tmp->id = 0;
                    $tmp->ftext = JText::_('COM_SURVEYFORCE_NO_ANSWER');
                    $fields[] = $tmp;
                ...
                case '3':
                    ...
                    $query = "SELECT b.ftext FROM #__survey_force_user_answers AS a, #__survey_force_fields AS b WHERE a.start_id = $start_id AND a.survey_id = $survey_id AND a.quest_id = $quest_id AND b.id = a.answer";
                    $database->SetQuery($query);
                    $usr_answers[1] = $database->loadRowList();
                ...
                case '6':
                case '9':
                    $query = "SELECT id, ftext from #__survey_force_fields WHERE quest_id = $quest_id AND is_main = 1 ORDER BY `ordering`";
                    $database->SetQuery($query);
                    $questions = $database->loadObjectList();

                    $query = "SELECT id, ftext AS stext from #__survey_force_fields WHERE quest_id = $quest_id AND is_main = 0 ORDER BY `ordering`";
                    $database->SetQuery($query);
                    $fields = $database->loadObjectList();
                    $tmp = new stdClass();
                    $tmp->id = 0;
                    $tmp->stext = ($qtype != 9 ? JText::_('COM_SURVEYFORCE_NO_ANSWER') : JText::_('COM_SURVEYFORCE_NOT_RANKED'));
                    $fields[] = $tmp;
    ...
    function createImage(&$sectionsz, &$titlesz, &$answersz, $maintitle, $max_value)
    {
        ...
        if (count($sectionsz) >= 1) {
            ...
            foreach ($sectionsz as $i => $section) {
                ...
                $html .= '<img src="' . $this->image_src . $filename . '" /><br/>';
            }
